...
function events:RegisterEvent(event, callback)
    ...
    GG:RegisterEvent(event, callback)
end
function events:RegisterMessage(event, callback ,cb)
    ...
    if not cb then GG:RegisterMessage(event, observerSendMessageNotify)
    else GG:RegisterMessage(event, callback) end
end
